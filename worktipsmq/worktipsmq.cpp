...
int WorktipsMQ::set_zmq_context_option(int option, int value) {
...
zmq::socket_t& WorktipsMQ::get_control_socket() {
    ...
    // Maps the WorktipsMQ unique ID to a local thread command socket.
...
void WorktipsMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection) {
...
void WorktipsMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection) {
    // TODO: As above.
    check_not_started(proxy_thread, "start listening");
...
WorktipsMQ::run_info& WorktipsMQ::run_info::load(batch_job&& bj, bool reply_job) {
    is_batch_job = true;
