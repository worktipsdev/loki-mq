#include "worktipsmq/batch.h"
#include "common.h"
#include <future>

TEST_CASE("tagged thread start functions", "[tagged][start]") {
    worktipsmq::WorktipsMQ lmq{get_logger(""), LogLevel::trace};
...
TEST_CASE("tagged threads quit-before-start", "[tagged][quit]") {
    auto lmq = std::make_unique<worktipsmq::WorktipsMQ>(get_logger(""), LogLevel::trace);
    auto t_abc = lmq->add_tagged_thread("abc");
